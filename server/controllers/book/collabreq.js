...
exports.sendReviewerRequest = async (req, res) => {
  try {
    ...
    const filteredEmails = emailarr.filter(
      (email) => !currReviewers.includes(email)
    );
    const totalemails = filteredEmails.length;
...
exports.acceptcollabInvitation = async (req, res) => {
  try {
    const checkvalidtoken = await Blacklisttoken.findOne({
      token: req.body.InviteLink,
    });
    if (checkvalidtoken) {
      return res
        .status(400)
        .json({ message: "Invite link has been blacklisted" });
    }
...
exports.declineinvitation = async (req, res) => {
  try {
    console.log("InviteLink");
    const existingBlacklist = await Blacklisttoken.findOne({
      token: req.body.InviteLink,
    });
    if (existingBlacklist) {
      return res
        .status(400)
        .json({ message: "Invite link is already blacklisted." });
    }
